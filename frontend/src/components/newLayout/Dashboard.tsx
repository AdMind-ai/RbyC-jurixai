...
const Dashboard: React.FC<DashboardProps> = ({ companies, deadlines, onAddDeadline }) => {
  ...
  return (
    <div className="w-full h-full p-8 overflow-y-auto animate-fade-in relative">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center border-b border-slate-300 pb-4 mb-6">
            <div>
            <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <Briefcase className="text-[#1e3a8a]" size={28} />
                Dashboard Segreteria
            </h2>
            <p className="text-slate-500">Panoramica attività e scadenze societarie</p>
            </div>
            <div className="flex gap-2">
                <div className="bg-white border border-slate-300 rounded-lg p-1 flex">
                    <button 
                        onClick={() => setViewMode('list')}
                        className={`p-2 rounded ${viewMode === 'list' ? 'bg-[#1e3a8a]/10 text-[#1e3a8a]' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                        <List size={20} />
                    </button>
                    <button 
                        onClick={() => setViewMode('calendar')}
                        className={`p-2 rounded ${viewMode === 'calendar' ? 'bg-[#1e3a8a]/10 text-[#1e3a8a]' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                        <Grid size={20} />
                    </button>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300 flex items-center justify-between">
            <div>
                <p className="text-sm font-medium text-slate-500">Società Gestite</p>
                <p className="text-3xl font-bold text-slate-800">{totalCompanies}</p>
            </div>
            <div className="p-3 bg-blue-50 text-[#1e3a8a] rounded-full border border-blue-100">
                <Users size={24} />
            </div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300 flex items-center justify-between">
            <div>
                <p className="text-sm font-medium text-slate-500">Scadenze Attive</p>
                <p className="text-3xl font-bold text-slate-800">{activeDeadlines}</p>
            </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300 flex items-center justify-between">
            <div>
                <p className="text-sm font-medium text-slate-500">Urgente (7gg)</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-300 flex flex-col h-[600px]">
            
            {viewMode === 'list' ? (
                <>
                    <div className="flex items-center gap-4 mb-6 border-b border-slate-200 pb-2 shrink-0">
                        <button 
                            onClick={() => setDeadlineFilter('urgent')}
                            className={`pb-2 text-sm font-medium transition-colors ${deadlineFilter === 'urgent' ? 'text-red-600 border-b-2 border-red-600' : 'text-slate-500 hover:text-slate-800'}`}
                        >
                            Urgenti / Scadute
                        </button>
                        <button 
                            onClick={() => setDeadlineFilter('upcoming')}
                            className={`pb-2 text-sm font-medium transition-colors ${deadlineFilter === 'upcoming' ? 'text-[#1e3a8a] border-b-2 border-[#1e3a8a]' : 'text-slate-500 hover:text-slate-800'}`}
                        >
                            Prossime
                        </button>
                        <button 
                            onClick={() => setDeadlineFilter('completed')}
                            className={`pb-2 text-sm font-medium transition-colors ${deadlineFilter === 'completed' ? 'text-green-600 border-b-2 border-green-600' : 'text-slate-500 hover:text-slate-800'}`}
                        >
                            Completate
                        </button>
                    </div>
                    ...
                    <div className="space-y-3 overflow-y-auto pr-2 flex-1">
                        ...
                        displayedDeadlines.map(d => {
                            ...
                            return (
                            <div key={d.id} className={`flex items-center gap-4 p-4 rounded-lg border transition-all ${d.completed ? 'bg-slate-50 border-slate-200 opacity-70' : 'bg-white border-slate-200 hover:border-blue-300 hover:shadow-sm'}`}>
                                <div 
                                    onClick={() => toggleComplete(d)}
                                    className={`cursor-pointer w-5 h-5 rounded border flex items-center justify-center ${d.completed ? 'bg-green-500 border-green-500 text-white' : 'border-slate-400 hover:border-[#1e3a8a]'}`}
                                >
                                ...
                                <div className="text-right">
                                    <div className={`text-sm font-semibold flex items-center gap-1 justify-end ${isOverdue ? 'text-red-600' : 'text-slate-700'}`}>
                                        <Clock size={14} />
                    ...
                    <div className="flex justify-between items-center mb-4 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="flex bg-slate-100 rounded-lg p-0.5 border border-slate-200">
                                <button onClick={() => changeMonth(-1)} className="p-1 hover:bg-white hover:shadow-sm rounded-md text-slate-500 transition-all">
                                    <ChevronLeft size={18} />
                                </button>
                                <button onClick={() => changeMonth(1)} className="p-1 hover:bg-white hover:shadow-sm rounded-md text-slate-500 transition-all">
                        ...
                        <div className="flex gap-2 text-xs">
                            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-blue-500 border border-blue-600"></div>Societario</div>
                            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-red-500 border border-red-600"></div>Fiscale</div>
                            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full bg-purple-500 border border-purple-600"></div>Legale</div>
                        </div>
                    </div>
                    
                    <div className="grid grid-cols-7 gap-2 text-center text-xs font-semibold text-slate-500 mb-2 shrink-0 uppercase tracking-wide">
                    ...
                    <div className="flex-1 grid grid-cols-7 gap-2 overflow-y-auto">
                        ...
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                            ...
                            return (
                                <div 
                                    key={day} 
                                    className={`border rounded-lg p-1 flex flex-col relative group min-h-[80px] transition-colors
                                ...
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <span className={`text-xs font-medium ${isToday ? 'text-blue-700 font-bold bg-blue-200 px-1.5 rounded-full' : (dayDeadlines.length > 0 ? 'text-[#1e3a8a]' : 'text-slate-500')}`}>
                                            {day}
                                        </span>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300 flex flex-col">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Tipologia Società</h3>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-300">
                <h3 className="text-lg font-semibold text-slate-800 mb-4">Statistiche Scadenze</h3>
                <div className="h-[200px]">
                    <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Tooltip cursor={{fill: '#f1f5f9'}} contentStyle={{backgroundColor: '#1e293b', border: 'none', borderRadius: '8px', color: '#fff'}} itemStyle={{color: '#fff'}} />
