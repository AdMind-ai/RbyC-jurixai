...
const PreviewDocument: React.FC<{ content: string; type: DraftType }> = ({ content, type }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div
      ...
      style={{
        background: 'linear-gradient(135deg, #1e3a8a 0%, #1e3a8a 60%, #1b9162 100%)',
      }}
    >
      <div>
    ...
    <div className="px-8 py-6 text-slate-700 text-[14px] leading-7">
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="text-2xl font-bold text-slate-800 mt-6 mb-3 first:mt-0">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-lg font-semibold text-slate-800 mt-5 mb-2 pb-1 border-b border-slate-100">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-base font-semibold text-slate-700 mt-4 mb-1.5">{children}</h3>
          ),
          p: ({ children }) => <p className="my-2 first:mt-0">{children}</p>,
          strong: ({ children }) => <strong className="font-semibold text-slate-800">{children}</strong>,
          ul: ({ children }) => (
            <ul className="my-2.5 space-y-1.5 pl-5 list-disc marker:text-[#1b9162]">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="my-2.5 space-y-1.5 pl-5 list-decimal marker:text-[#1b9162] marker:font-semibold">
              {children}
            </ol>
          ),
          li: ({ children }) => <li className="pl-1">{children}</li>,
          blockquote: ({ children }) => (
            <blockquote className="my-3 border-l-4 border-[#1b9162] pl-4 text-slate-600 bg-green-50/50 py-2 rounded-r-lg">
              {children}
            </blockquote>
          ),
          hr: () => <hr className="my-5 border-slate-100" />,
          code: ({ children }) => (
            <code className="rounded bg-slate-100 px-1.5 py-0.5 font-mono text-[12px]">{children}</code>
          ),
        }}
      >
        {content}
                ...
                {msg.files && msg.files.length > 0 && (
                  <div className={`flex flex-wrap gap-1 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                    {msg.files.map((f, i) => (
                      <span
                        key={i}
                        className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg px-2 py-1 text-[11px] text-slate-500"
                      >
                        <FileText size={10} />
                        {f.name}
        ...
        {attachedFiles.length > 0 && (
          <div className="px-5 pt-2 flex flex-wrap gap-1.5 shrink-0">
            {attachedFiles.map((f, i) => (
              <span
                key={i}
                className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 text-[#1e3a8a] rounded-lg px-2.5 py-1 text-[11px]"
              >
                <FileText size={10} />
                {f.name}
        ...
        <div className="px-5 py-4 border-t border-slate-100 shrink-0">
          <div className="relative flex items-end gap-2">
            <button
              ...
              className="shrink-0 p-2 text-slate-400 hover:text-[#1e3a8a] transition-colors rounded-lg hover:bg-slate-100"
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center">
              <div className="relative">
                ...
                <div
                  ...
                  style={{ background: 'linear-gradient(135deg, #1b9162, #4ade80)' }}
                >
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                {(draftType === 'newsletter'
                  ? [
                      'Crea una newsletter sulle novità del D.Lgs. 231/01',
                      'Genera una newsletter sul GDPR aggiornato',
                      'Scrivi una newsletter su antiriciclaggio per il mese di luglio',
                    ]
                  : [
                      'Crea un PILL formativo sulla responsabilità d\'impresa',
                      'Genera un PILL sul whistleblowing',
                      'Scrivi un PILL formativo sulla privacy in azienda',
                    ]
                ).map((ex) => (
                  <button
                    key={ex}
                    onClick={() => setInput(ex)}
                    className="text-[12px] text-[#1e3a8a] bg-blue-50 hover:bg-blue-100 rounded-xl px-4 py-2 text-left transition-colors"
                  >
                    "{ex}"
                  </button>
                ))}
